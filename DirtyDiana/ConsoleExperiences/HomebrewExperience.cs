...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        private static void DisplayApps(List<HomebrewApp> apps)
        {
            ...
            {
                // Remove the first two directories from the folder path
                string[] folderParts = app.folder.Split(new[] { '/', '\\' }, StringSplitOptions.RemoveEmptyEntries);
                string strippedFolder = string.Join(
                    Path.DirectorySeparatorChar.ToString(),
                    folderParts.Skip(2)
                );

                // fallback
                if (string.IsNullOrEmpty(strippedFolder))
                    strippedFolder = app.folder;

                table.AddRow($"[#A1CF3E]{strippedFolder}[/]", $"[#ffac4d]{Path.GetFileName(app.entryPoint)}[/]");
            }
        ...
        private static void RemoveHomebrewApp(List<HomebrewApp> apps)
        {
            ...
            var appToRemove = AnsiConsole.Prompt(
                ...
                .AddChoices(apps.Select(app => app.name))
            );

            var selectedApp = apps.First(app => app.name == appToRemove);
            apps.Remove(selectedApp);

            AnsiConsole.MarkupLine($"[#ffac4d]Removed:[/] {selectedApp.name}\n");
        }
    }
}
